...
Configure **`scripts/nodejs/.env`** (`TOKEN`, optional OAuth IDs for REST). Outputs land in **`scripts/nodejs/repos/`** and **`scripts/nodejs/graphQlData/`** (see sibling READMEs under `data/sources/github_rest_api` and `data/sources/github_graphql_api`).
...
There is **no** single static URL for “the API dataset.” The manifest marks this source as **`manual`**; run the Node pipeline or your own batch job, then place derived CSVs under `data/downloads/github_api_live/` if you want them beside other pulled artifacts.
