...
Output files are produced by the Node process; they are **not** listed in `data/manifest.json` because they are generated locally, not downloaded from a static URL. Treat `scripts/nodejs/repos/*.csv` as **REST API** lineage when combining with GraphQL-enriched data.
